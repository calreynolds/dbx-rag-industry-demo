...
def create(app_name, app_description = "This app does something"):
  requests.post(f"{host}/api/2.0/preview/apps", headers=auth, json={
  "name": app_name,
  "spec": {
  ...
  for _ in range(10):
      time.sleep(5)
      response = requests.get(f"{host}/api/2.0/preview/apps/{app_name}", headers=auth).json()
...
def deploy(app_name, source_code_path):
  ...
  response = requests.post(f"{host}/api/2.0/preview/apps/{app_name}/deployments", headers=auth, json={
  ...
  for _ in range(10):
      time.sleep(5)
      response = requests.get(f"{host}/api/2.0/preview/apps/{app_name}/deployments/{deployment_id}", headers=auth).json()
